...
## Актуализация по текущему uncommitted diff

Проверены staged, unstaged и untracked изменения на 2026-07-26. В текущей
версии Milestone 4 реализован: PDF записывается через временный файл,
поддерживаются `--overwrite`, `--quiet`, `--check`, stdin и сообщения об
успехе. Проверки прошли: `cargo test --all-targets --all-features` (117 unit,
9 CLI, 16 compiler, 14 end-to-end, 41 markdown parser, 15 typst generator) и
`cargo clippy --all-targets --all-features -- -D warnings`.

### Подтверждённые findings текущего diff

#### P1 — удаление старого PDF при ошибке замены на Windows

| | |
|---|---|
| **Где** | `src/output.rs:79-85` (`replace`) |
| **Сценарий** | Первый `rename` завершается ошибкой из-за существующего target; код удаляет старый PDF, а второй `rename` затем не удаётся (например, target заблокирован другим процессом или вмешалось ПО безопасности). |
| **Результат** | Функция возвращает ошибку, но прежний PDF уже потерян. Это противоречит гарантии в документации функции и `docs/progress.md`, что существующий файл не страдает при ошибке. |
| **Рекомендация** | Использовать безопасную стратегию замены для Windows с сохранением rollback/backup до подтверждённого переименования; отдельно покрыть отказ второго `rename` тестом или платформенным адаптером. |

#### P2 — гонка в запрете молчаливой перезаписи

| | |
|---|---|
| **Где** | `src/output.rs:26-30`, затем `replace` |
| **Сценарий** | Два параллельных запуска одновременно видят отсутствующий output после
  `path.exists()`, оба компилируют и оба успешно переименовывают временный файл. |
| **Результат** | Последний запуск молча заменяет результат первого при
  `overwrite == false`, хотя контракт требует запретить перезапись без флага. |
| **Рекомендация** | Сделать резервирование назначения атомарным либо использовать
  операцию no-replace/exclusive-create, чтобы проверка и запись не разделялись
  гонкой. Добавить конкурентный интеграционный тест. |

Эти два пункта являются единственными actionable findings, подтверждёнными в
текущем diff. Остальная часть отчёта ниже отражает предыдущий baseline и
сохраняется как исторический контекст; статусы из разделов 1 и 4.1 нужно
считать устаревшими относительно актуализации выше.
