...
/// Лимиты структуры документа (ТЗ §40).
///
/// Живут в `ast`, а не в `compiler`: их обязан соблюдать парсер, а зависеть
/// от слоя компиляции ему нельзя (AGENTS.md, таблица границ).
pub mod limits {
    /// Максимальное число узлов AST.
    pub const MAX_AST_NODES: usize = 1_000_000;
    /// Максимальная глубина вложенности контейнеров.
